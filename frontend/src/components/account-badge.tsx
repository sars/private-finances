 ...
 * shows the member as a ring, so it still reads in a dense row.
 */
export function AccountBadge({
  ...
  const fill = Array.isArray(tile.fill) ? `url(#${gradientId})` : tile.fill;
  const description = [
    identity.name,
  ...
  return (
    ...
    >
      {Array.isArray(tile.fill) ? (
        <defs>
          <linearGradient id={gradientId} x1="0" y1="0" x2="0" y2="1">
            <stop offset="0" stopColor={tile.fill[0]} />
            <stop offset="1" stopColor={tile.fill[1]} />
          </linearGradient>
        </defs>
      ) : null}
      <rect
