...
- [ ] **PF-020 / AS-4: feeds.** IBKR Flex Web Service, Binance Spot read-only,
  BTC and ETH wallets by address with a public price source.
...
- [x] **PF-003 / CAT-3: family-transfer evidence.** Built and deployed at schema
  ...
  **Both-sides corroboration is rejected, not outstanding.** It was built,
  ...
  the printed number is the *other* party's card. The path is kept and is right
...
- [ ] **REC-3: optional item allocations and reports.** Split a purchase into
  ...
  receipt/fiscal-check endpoints that do exist belong to the *acquiring* API:
