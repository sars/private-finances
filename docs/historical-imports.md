...
| LHV EE      | Between two and six months, measured 17 September 2026 | A window starting 17 March 2026 or earlier is refused outright with HTTP 422 `WRONG_TRANSACTIONS_PERIOD` ("Requested time period is out of bounds"), which the connector reports as `schema`; a window from 17 July 2026 is served. The bank also enforces the PSD2 allowance on unattended calls: a handful of requests in a row answered 429 "Access exceeded", so LHV runs on the six-hour cadence (no `half-hourly` marker) and a backfill must stay inside its window and its allowance. The provider lists the bank as "LHV Pank", personal accounts, redirect approval, marked beta on its side, approval valid up to 180 days. Its account holds several currencies (`XXX`). |
